...
! local:
  ...
  real, pointer, dimension(:,:,:) :: sin_sg, agrid
  real, pointer, dimension(:,:)   :: rarea, area
  real(kind=R_GRID), pointer, dimension(:,:,:) :: agrid_64
  real(kind=R_GRID), pointer, dimension(:,:)   :: area_64
  ...
  agrid_64 => gridstruct%agrid_64
   area_64 => gridstruct%area_64
  ...
  do j=js,je
     ...
     enddo
  enddo
  if ( tc_mask )  call get_tc_mask(time, mask, bd, agrid_64)
...
! local
      ...
      real(kind=R_GRID), pointer :: area(:,:), agrid(:,:,:)
...
! local
      ...
      real(kind=R_GRID), pointer :: vlon(:,:,:), vlat(:,:,:), agrid_64(:,:,:), area_64(:,:)
...
!----------------------------------------
      ...
      do j=js, je
         do i=is, ie
            dist(i,j) = great_circle_dist( pos, agrid_64(i,j,1:2), radius )
         enddo
      enddo
      ...
      do j=js, je
         do i=is, ie
            if( dist(i,j) <= min(r_vor, r_fac*r_max) .and. phis(i,j)<2.*grav ) then
                ...
                call tangent_wind(vlon(1:3,i,j), vlat(1:3,i,j), speed_local, pos, agrid_64(i,j,1:2), ut, vt)
                ut = ut + u_bg
                vt = vt + v_bg
