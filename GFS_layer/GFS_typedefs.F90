...
!----------------------------------------------------------------
  type cosp_type
    real (kind=kind_phys), pointer :: longitude                          (:)   => null()
    real (kind=kind_phys), pointer :: latitude                           (:)   => null()
    real (kind=kind_phys), pointer :: tau7                               (:)   => null()
    real (kind=kind_phys), pointer :: tau7_bnds                          (:,:)   => null()
    real (kind=kind_phys), pointer :: pres7                              (:)   => null()
    real (kind=kind_phys), pointer :: pres7_bnds                         (:,:)   => null()
    real (kind=kind_phys), pointer :: hgt16                              (:)   => null()
    real (kind=kind_phys), pointer :: hgt16_bnds                         (:,:)   => null()
    real (kind=kind_phys), pointer :: atb532_perp                        (:,:,:)   => null()
    real (kind=kind_phys), pointer :: atb532                             (:,:,:)   => null()
    real (kind=kind_phys), pointer :: calipso_tau                        (:,:,:)   => null()
    real (kind=kind_phys), pointer :: cfadLidarsr532                     (:,:,:)   => null()
    real (kind=kind_phys), pointer :: clcalipso                          (:,:)   => null()
    real (kind=kind_phys), pointer :: lidarBetaMol532                    (:,:)   => null()
    real (kind=kind_phys), pointer :: SR_EDGES                           (:,:)   => null()
    real (kind=kind_phys), pointer :: parasolPix_refl                    (:,:,:)   => null()
    real (kind=kind_phys), pointer :: parasolGrid_refl                   (:,:)   => null()
    real (kind=kind_phys), pointer :: dbze94                             (:,:,:)   => null()
    real (kind=kind_phys), pointer :: cfadDbze94                         (:,:,:)   => null()
    ...
    real (kind=kind_phys), pointer :: boxtauisccp                        (:,:)   => null()
    real (kind=kind_phys), pointer :: boxptopisccp                       (:,:)   => null()
    real (kind=kind_phys), pointer :: clisccp                            (:,:,:)   => null()
    real (kind=kind_phys), pointer :: clMISR                             (:,:,:)   => null()
    ...
    real (kind=kind_phys), pointer :: clmodis                            (:,:,:)   => null()
    real (kind=kind_phys), pointer :: modis_Optical_Thickness_vs_ReffICE (:,:,:)   => null()
    real (kind=kind_phys), pointer :: modis_Optical_Thickness_vs_ReffLIQ (:,:,:)   => null()
    real (kind=kind_phys), pointer :: clcalipso2                         (:,:)   => null()
    real (kind=kind_phys), pointer :: cltlidarradar                      (:)   => null()
    real (kind=kind_phys), pointer :: clcalipsoice                       (:,:)   => null()
    real (kind=kind_phys), pointer :: clcalipsoliq                       (:,:)   => null()
    real (kind=kind_phys), pointer :: clcalipsoun                        (:,:)   => null()
    ...
    real (kind=kind_phys), pointer :: clcalipsotmp                       (:,:)   => null()
    real (kind=kind_phys), pointer :: clcalipsotmpice                    (:,:)   => null()
    real (kind=kind_phys), pointer :: clcalipsotmpliq                    (:,:)   => null()
    real (kind=kind_phys), pointer :: clcalipsotmpun                     (:,:)   => null()
    real (kind=kind_phys), pointer :: SR_BINS_list                       (:)   => null()
    real (kind=kind_phys), pointer :: bnds                               (:)   => null()
    real (kind=kind_phys), pointer :: loc                                (:)   => null()
    real (kind=kind_phys), pointer :: lev                                (:)   => null()
    real (kind=kind_phys), pointer :: levStat                            (:)   => null()
    real (kind=kind_phys), pointer :: cosp_scol                          (:)   => null()
    real (kind=kind_phys), pointer :: PARASOL_NREFL_list                 (:)   => null()
    real (kind=kind_phys), pointer :: cloudsat_DBZE_BINS_list            (:)   => null()
    real (kind=kind_phys), pointer :: REICE_MODIS                        (:)   => null()
    real (kind=kind_phys), pointer :: RELIQ_MODIS                        (:)   => null()
    ...
    real (kind=kind_phys), pointer :: clcalipsoopaque                    (:,:)   => null()
    real (kind=kind_phys), pointer :: clcalipsothin                      (:,:)   => null()
    real (kind=kind_phys), pointer :: clcalipsozopaque                   (:,:)   => null()
    real (kind=kind_phys), pointer :: clcalipsoopacity                   (:,:)   => null()
    ...
    real (kind=kind_phys), pointer :: clgrLidar532                       (:,:)   => null()
    real (kind=kind_phys), pointer :: lidarBetaMol532gr                  (:,:)   => null()
    real (kind=kind_phys), pointer :: cfadLidarsr532gr                   (:,:,:)   => null()
    real (kind=kind_phys), pointer :: atb532gr                           (:,:,:)   => null()
    real (kind=kind_phys), pointer :: SR_BINS_GR                         (:)   => null()
    real (kind=kind_phys), pointer :: SR_EDGES_GR                        (:,:)   => null()
    ...
    real (kind=kind_phys), pointer :: clatlid                            (:,:)   => null()
    real (kind=kind_phys), pointer :: lidarBetaMol355                    (:,:)   => null()
    real (kind=kind_phys), pointer :: cfadLidarsr355                     (:,:,:)   => null()
    real (kind=kind_phys), pointer :: atb355                             (:,:,:)   => null()
    real (kind=kind_phys), pointer :: SR_BINS_ATLID                      (:)   => null()
    real (kind=kind_phys), pointer :: SR_EDGES_ATLID                     (:,:)   => null()
    ...
    real (kind=kind_phys), pointer :: ncfodd1                            (:,:,:)   => null()
    real (kind=kind_phys), pointer :: ncfodd2                            (:,:,:)   => null()
    real (kind=kind_phys), pointer :: ncfodd3                            (:,:,:)   => null()
    real (kind=kind_phys), pointer :: CFODD_NDBZE_list                   (:)   => null()
    real (kind=kind_phys), pointer :: CFODD_NICOD_list                   (:)   => null()
  end type cosp_type
...
!----------------
  subroutine diag_create (Diag, IM, Model)
    ...
    if (Model%do_cosp) then
      allocate (Diag%cosp%longitude                          (IM))
      allocate (Diag%cosp%latitude                           (IM))
      allocate (Diag%cosp%tau7                               (ntau))
      allocate (Diag%cosp%tau7_bnds                          (2,ntau))
      allocate (Diag%cosp%pres7                              (npres))
      allocate (Diag%cosp%pres7_bnds                         (2,npres))
      allocate (Diag%cosp%hgt16                              (nhgt))
      allocate (Diag%cosp%hgt16_bnds                         (2,nhgt))
      allocate (Diag%cosp%atb532_perp                        (IM,Ncolumns,Model%levs))
      allocate (Diag%cosp%atb532                             (IM,Ncolumns,Model%levs))
      allocate (Diag%cosp%calipso_tau                        (IM,Ncolumns,Model%levs))
      allocate (Diag%cosp%cfadLidarsr532                     (IM,SR_BINS,Nlvgrid))
      allocate (Diag%cosp%clcalipso                          (IM,Nlvgrid))
      allocate (Diag%cosp%lidarBetaMol532                    (IM,Model%levs))
      allocate (Diag%cosp%SR_EDGES                           (2,SR_BINS))
      allocate (Diag%cosp%parasolPix_refl                    (IM,Ncolumns,PARASOL_NREFL))
      allocate (Diag%cosp%parasolGrid_refl                   (IM,PARASOL_NREFL))
      allocate (Diag%cosp%dbze94                             (IM,Ncolumns,Model%levs))
      allocate (Diag%cosp%cfadDbze94                         (IM,cloudsat_DBZE_BINS,Nlvgrid))
      ...
      allocate (Diag%cosp%boxtauisccp                        (IM,Ncolumns))
      allocate (Diag%cosp%boxptopisccp                       (IM,Ncolumns))
      allocate (Diag%cosp%clisccp                            (IM,ntau,npres))
      allocate (Diag%cosp%clMISR                             (IM,ntau,nhgt))
      ...
      allocate (Diag%cosp%clmodis                            (IM,ntau,npres))
      allocate (Diag%cosp%modis_Optical_Thickness_vs_ReffICE (IM,ntau,numMODISReffIceBins))
      allocate (Diag%cosp%modis_Optical_Thickness_vs_ReffLIQ (IM,ntau,numMODISReffLiqBins))
      allocate (Diag%cosp%clcalipso2                         (IM,Nlvgrid))
      allocate (Diag%cosp%cltlidarradar                      (IM))
      allocate (Diag%cosp%clcalipsoice                       (IM,Nlvgrid))
      allocate (Diag%cosp%clcalipsoliq                       (IM,Nlvgrid))
      allocate (Diag%cosp%clcalipsoun                        (IM,Nlvgrid))
      ...
      allocate (Diag%cosp%clcalipsotmp                       (IM,Nlvgrid))
      allocate (Diag%cosp%clcalipsotmpice                    (IM,Nlvgrid))
      allocate (Diag%cosp%clcalipsotmpliq                    (IM,Nlvgrid))
      allocate (Diag%cosp%clcalipsotmpun                     (IM,Nlvgrid))
      allocate (Diag%cosp%SR_BINS_list                       (SR_BINS))
      allocate (Diag%cosp%bnds                               (2))
      allocate (Diag%cosp%loc                                (IM))
      allocate (Diag%cosp%lev                                (Model%levs))
      ...
      allocate (Diag%cosp%clcalipsoopaque                    (IM,Nlvgrid))
      allocate (Diag%cosp%clcalipsothin                      (IM,Nlvgrid))
      allocate (Diag%cosp%clcalipsozopaque                   (IM,Nlvgrid))
      allocate (Diag%cosp%clcalipsoopacity                   (IM,Nlvgrid))
      ...
      allocate (Diag%cosp%clgrLidar532                       (IM,Nlvgrid))
      allocate (Diag%cosp%lidarBetaMol532gr                  (IM,Model%levs))
      allocate (Diag%cosp%cfadLidarsr532gr                   (IM,SR_BINS,Nlvgrid))
      allocate (Diag%cosp%atb532gr                           (IM,Ncolumns,Model%levs))
      allocate (Diag%cosp%SR_BINS_GR                         (SR_BINS))
      allocate (Diag%cosp%SR_EDGES_GR                        (2,SR_BINS))
      allocate (Diag%cosp%cllatlid                           (IM))
      allocate (Diag%cosp%clmatlid                           (IM))
      allocate (Diag%cosp%clhatlid                           (IM))
      allocate (Diag%cosp%cltatlid                           (IM))
      allocate (Diag%cosp%clatlid                            (IM,Nlvgrid))
      allocate (Diag%cosp%lidarBetaMol355                    (IM,Model%levs))
      allocate (Diag%cosp%cfadLidarsr355                     (IM,SR_BINS,Nlvgrid))
      allocate (Diag%cosp%atb355                             (IM,Ncolumns,Model%levs))
      allocate (Diag%cosp%SR_BINS_ATLID                      (SR_BINS))
      allocate (Diag%cosp%SR_EDGES_ATLID                     (2,SR_BINS))
      ...
      allocate (Diag%cosp%ncfodd1                            (IM,CFODD_NDBZE,CFODD_NICOD))
      allocate (Diag%cosp%ncfodd2                            (IM,CFODD_NDBZE,CFODD_NICOD))
      allocate (Diag%cosp%ncfodd3                            (IM,CFODD_NDBZE,CFODD_NICOD))
      allocate (Diag%cosp%CFODD_NDBZE_list                   (CFODD_NDBZE))
      allocate (Diag%cosp%CFODD_NICOD_list                   (CFODD_NICOD))
    endif
