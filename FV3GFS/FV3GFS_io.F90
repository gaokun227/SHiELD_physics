...
!--- physics instantaneous diagnostics ---
    ...
    Diag(idx)%axes = 3
    Diag(idx)%name = 'delt'
    Diag(idx)%desc = 'temperature change due to terrain induced lifting'
    Diag(idx)%unit = 'K'
    Diag(idx)%mod_name = 'gfs_phys'
    Diag(idx)%time_avg = .TRUE.
    allocate (Diag(idx)%data(nblks))
    do nb = 1,nblks
      Diag(idx)%data(nb)%var3 => Gfs_diag(nb)%delt(:,:)
    enddo

    idx = idx + 1
    Diag(idx)%axes = 3
    Diag(idx)%name = 'delqv'
    Diag(idx)%desc = 'water vapor change due to terrain induced lifting'
    Diag(idx)%unit = 'K'
    Diag(idx)%mod_name = 'gfs_phys'
    Diag(idx)%time_avg = .TRUE.
    allocate (Diag(idx)%data(nblks))
    do nb = 1,nblks
      Diag(idx)%data(nb)%var3 => Gfs_diag(nb)%delqv(:,:)
    enddo

    idx = idx + 1
    Diag(idx)%axes = 3
    Diag(idx)%name = 'delql'
    Diag(idx)%desc = 'cloud water change due to terrain induced lifting'
    Diag(idx)%unit = 'K'
    Diag(idx)%mod_name = 'gfs_phys'
    Diag(idx)%time_avg = .TRUE.
    allocate (Diag(idx)%data(nblks))
    do nb = 1,nblks
      Diag(idx)%data(nb)%var3 => Gfs_diag(nb)%delql(:,:)
    enddo

    idx = idx + 1
    Diag(idx)%axes = 3
    Diag(idx)%name = 'delqi'
    Diag(idx)%desc = 'cloud ice change due to terrain induced lifting'
    Diag(idx)%unit = 'K'
    Diag(idx)%mod_name = 'gfs_phys'
    Diag(idx)%time_avg = .TRUE.
    allocate (Diag(idx)%data(nblks))
    do nb = 1,nblks
      Diag(idx)%data(nb)%var3 => Gfs_diag(nb)%delqi(:,:)
    enddo

    idx = idx + 1
    Diag(idx)%axes = 3
    Diag(idx)%name = 'delqr'
    Diag(idx)%desc = 'rain change due to terrain induced lifting'
    Diag(idx)%unit = 'K'
    Diag(idx)%mod_name = 'gfs_phys'
    Diag(idx)%time_avg = .TRUE.
    allocate (Diag(idx)%data(nblks))
    do nb = 1,nblks
      Diag(idx)%data(nb)%var3 => Gfs_diag(nb)%delqr(:,:)
    enddo

    idx = idx + 1
    Diag(idx)%axes = 3
    Diag(idx)%name = 'delqs'
    Diag(idx)%desc = 'snow change due to terrain induced lifting'
    Diag(idx)%unit = 'K'
    Diag(idx)%mod_name = 'gfs_phys'
    Diag(idx)%time_avg = .TRUE.
    allocate (Diag(idx)%data(nblks))
         ...
         !---
           ...
           do j = 1, ny
             ...
             do i = 1, nx
               ...
               nb = Atm_block%blkno(ii,jj)
               ix = Atm_block%ixp(ii,jj)
               var3(i,j,:) = Diag(idx)%data(nb)%var3(ix,levs:1:-1)*lcnvfac
             enddo
           enddo
