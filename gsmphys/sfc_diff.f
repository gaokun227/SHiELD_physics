...
! aug 2019 - an updated and clean version by kgao (add ztrl)
...
!
     ...
     &                     u10m, v10m, ws10m, z0_1, ustar_1 !kgao
!
...
!
! modify zt over ocean by kgao
! zt is now allowed to decouple from zo under high winds
!
            z0_1 = (charnock / grav) * ustar(i) * ustar(i)
            if (redrag) then
              z0_1 = max(min(z0, z0s_max), 1.e-7)
            else
              z0_1 = max(min(z0,.1), 1.e-7)
            endif 
! zeng, zhao and dickinson 1997 (eq 25)
            ustar_1 = sqrt(grav * z0_1 / charnock)  
            restar = max(ustar_1*z0_1*visi, 0.000001)
            rat    = min(7.0, 2.67 * sqrt(sqrt(restar)) - 2.57)
            ztrl(i)  = 100.0 * z0_1 * exp(-rat)
